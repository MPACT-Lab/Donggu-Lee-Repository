            ...
            for soft_num=1:4
                if displaySimulationInformation
                    ...
                    disp(['Subframe: ' num2str(subframeNo)...                                        
                                    '. Soft LLR IDX: ' num2str(soft_num) ...                          
                           'SNRIdx: ' num2str(snrIdx)]);
                end
                ...
                if iscell(soft_LLR_buffer)
                    soft_LLR_buffer = soft_LLR_buffer{:, :};
                end
                ...
                if ~isempty(soft_LLR_buffer)
                    if length(soft_LLR_buffer) > length(dlschBits)
                        dlschBits = [dlschBits; zeros(length(soft_LLR_buffer) - length(dlschBits), 1)];
                        
                    elseif length(soft_LLR_buffer) < length(dlschBits)
                        soft_LLR_buffer = [soft_LLR_buffer; zeros(length(dlschBits) - length(soft_LLR_buffer), 1)];

                    end
                    dlschBits = soft_LLR_buffer + dlschBits;
